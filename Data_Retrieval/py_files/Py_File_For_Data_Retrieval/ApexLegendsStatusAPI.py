...
def get_player_stats(uid, platform):
    url = 'https://api.mozambiquehe.re/bridge'
    params = {
        'auth': API_KEY,
        'uid': uid,
...
def process_uids_from_csv(input_csv, platform, output_csv, batch_size=100, wait_time=300):
    with open(input_csv, newline='', encoding='utf-8') as csvfile:
        reader = csv.reader(csvfile)
        next(reader)  # Skip header if it exists
        players = [(row[0], row[1]) for row in reader]  # Assuming username is in column 0 and UID is in column 1

    # Open the CSV file to append data
    with open(output_csv, mode='w', newline='', encoding='utf-8') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['player_name', 'career_kills', 'career_wins', 'career_revives', 'legend', 'BR Kills', 'BR Damage', 'BR Wins'])  # Write header
        ...
        for i in range(0, len(players), batch_size):
            ...
            for username, uid in batch:
                ...
                if player_stats:
                    relevant_stats = extract_relevant_stats(player_stats)
                    relevant_stats['player_name'] = username  # Add player name to the stats
                    write_stats_to_csv(relevant_stats, writer)
                print("Waiting 5 seconds to fetch next player's data...")
...
def write_stats_to_csv(stats, writer):
    player_name = stats.get('player_name', '')
    career_kills = stats.get('career', {}).get('career_kills', 0)
    career_wins = stats.get('career', {}).get('career_wins', 0)
    career_revives = stats.get('career', {}).get('career_revives', 0)
...
# Example usage
input_csv = 'Data_Retrieval/CSV_files/Usernames/shortened_version_Xbox.csv'  # CSV file containing the list of player usernames and UIDs
platform = 'X1'  # Platform for which data is being fetched
output_csv = 'Data_Retrieval/CSV_files/Uncleaned_shortened_version_Xbox.csv'  # Output CSV file for storing the combined data
